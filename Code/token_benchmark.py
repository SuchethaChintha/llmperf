...
def get_token_throughput_latencies(
    ...
    with open("Models.json") as c:
        sub_details = json.load(c)
    for s in sub_details['config']:
        subscription_id=s['subscription_id']
        resource_group=s['resource_group']
        workspaces=s['workspaces']
        region=s['region']
        Provider_Platform=s['Provider_Platform']
    ...
    while (
        ...
        if not (iter % num_concurrent_requests):
            ...
            for out in outs:
                ...
                request_metrics["source_mean_input_tokens"]=mean_input_tokens
                request_metrics["source_stddev_input_tokens"]=stddev_input_tokens
                request_metrics["source_mean_output_tokens"]=mean_output_tokens
                request_metrics["source_stddev_output_tokens"]=stddev_output_tokens
                request_metrics["source_num_concurrent_requests"]=num_concurrent_requests
                request_metrics["source_max_num_completed_requests"]=max_num_completed_requests
                ...
                request_metrics["region"]=region
                request_metrics["Provider_Platform"]=Provider_Platform
    ...
    metadata = {
        ...
        "source_mean_input_tokens": mean_input_tokens,
        "source_stddev_input_tokens": stddev_input_tokens,
        "source_mean_output_tokens": mean_output_tokens,
        "source_stddev_output_tokens": stddev_output_tokens,
        "source_num_concurrent_requests": num_concurrent_requests,
        "source_max_num_completed_requests": max_num_completed_requests,
...
def run_token_benchmark(
    ...
    if results_dir:
        ...
        # (Path(args.llmperf_output) / results_dir)
        # llmperf_output="https://testeaststoragee15ba0db4.file.core.windows.net/llmperf/llmperf/llmperf_results/"
        # results_dir = os.path.join(llmperf_output,results_dir)
        results_dir =(Path(args.results_dir) / results_dir)
        # results_dir =Path(args.results_dir)
        results_dir.mkdir(parents=True, exist_ok=True)
        summary_filename = results_dir / f"{filename}_summary.json"
        individual_responses_filename = results_dir / f"{filename}_individual_responses.json"
        ...
        try:
            ...
            # d=json.load(str(results.to_dict()))
            # (Path(args.results_dir) / f"{model}_{mean_input_tokens}_{mean_output_tokens}_summary.json").write_text(results.to_dict())
        except Exception as e:
            print(results.to_dict())
            raise e

        try:
            with individual_responses_filename.open("w") as f:
                json.dump(individual_responses, f, indent=4)
            # (Path(args.results_dir) / f"{model}_{mean_input_tokens}_{mean_output_tokens}individual_responses_filename.json").write_text(str(individual_responses))
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # ray.init(runtime_env={"env_vars": env_vars, "pip_packages": ["ray[default]"]})
    args = argparse.ArgumentParser(
    description="Run a token throughput and latency benchmark."
    )
    args.add_argument(
    ...
    args.add_argument(
    "--results_dir",  help="Path of output llmperf result"
    )
    args.add_argument(
    "--mean-input-tokens",
    type=int,
